...
void ESParaSite::Sensors::readDhtSensor(bool inLoopRead, bool print) {
  ...
  if (exists.dhtDetected == 1) {
    ...
    switch (status) {
    case DHT12_OK:
      chamber.chamberTempC = ESParaSite::Util::floatToInt(dht.getTemperature());
      break;
    case DHT12_ERROR_CHECKSUM:
      Serial.print(F("DHT12 Checksum error,\t"));
      break;
    ...
    if (print == true) {
      ...
      Serial.print((chamber.chamberTempC / 100));
      Serial.print("°C / ");
      Serial.print(Util::convertCtoF((chamber.chamberTempC / 100)));
      Serial.println("°F");
    }
  } else {
